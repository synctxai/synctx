...
| `createDeal(...)` | `address partyB, uint96 grossAmount, address verifier, uint96 verifierFee, uint256 deadline, bytes sig, string tweet_id` | `uint256 dealIndex` | XQuoteDealContract | XQuoteDealContract | Creates a deal. Calls `XQuoteVerifierSpec.check()` for verifier signature verification. Quoter binding is verified off-chain by the verifier when signing. Business parameters are passed as flat arguments |
...
```
XQuoteVerifierSpec.check(verifierInstance, tweet_id, fee, deadline, sig)
  │
  ├── 1. Construct structHash(tweet_id, fee, deadline)
...
> **Note:** Quoter identity (userId) is no longer on-chain. The Verifier resolves it via Platform API (`POST /twitter/binding/verify`).
...
sequenceDiagram
    ...
    A->>P: 🟣 request_sign(verifier_address, params, deadline)<br/>params = {tweet_id, quoter_address}
    ...
    A->>D: 🟢 createDeal(partyB, grossAmount, verifier, verifierFee, deadline, sig, tweet_id) → dealIndex
...
| 2 | Verifier signature: calls `XQuoteVerifierSpec.check(verifier, tweet_id, verifierFee, deadline, sig)` | revert SignatureExpired / InvalidSignature / InvalidVerifierSignature |
