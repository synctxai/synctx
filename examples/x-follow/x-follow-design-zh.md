...
- **支付代币：** USDC
- **标签：** `["x", "follow"]`
- **验证语义：** 弱语义 — 验证者检查**验证时刻**关注关系是否存在，而非该关注是否在交易创建之后建立。签名阶段前置检查：B 必须已通过 `TwitterRegistry` 认证 Twitter 身份，或尚未关注目标账号。
...
sequenceDiagram
    ...
    Note over A: ⚠️ 前置检查：B 已通过 TwitterRegistry 认证 Twitter 身份，<br/>或尚未关注 target_username
    ...
    Note over V: 前置检查：TwitterRegistry.getAddressByUsername(follower) != 0x0<br/>或双源检查确认尚未关注
    V->>P: 🟣 send_message 回复
...
| 5 | TwitterRegistry 已认证或尚未关注（签名阶段） | 拒绝签名请求 |
