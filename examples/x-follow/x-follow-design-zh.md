...
- **支付代币：** USDC
- **标签：** `["x", "follow"]`
- **验证语义：** 弱语义 — 验证者检查**验证时刻**关注关系是否存在。由于 `TwitterRegistry` 绑定为强制要求，B 的身份得到保证（钱包 ↔ 用户名），消除了冒领风险。无需在签名阶段检查关注状态 — 如果 B 已经在关注，这属于 A 和 B 在交易前协商的范畴。
...
sequenceDiagram
    ...
    Note over A: ⚠️ B 必须已通过 TwitterRegistry 认证 Twitter 身份
    ...
    Note over V: 前置检查：TwitterRegistry.getAddressByUsername(follower) != 0x0
    V->>P: 🟣 send_message 回复
...
| 5 | TwitterRegistry 已认证（签名阶段，强制要求） | 拒绝签名请求 |
