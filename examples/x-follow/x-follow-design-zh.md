...
| 方法 | 参数 | 调用者 | 说明 |
|------|------|--------|------|
| `createDeal(...)` | `uint96 grossAmount, address verifier, uint96 verifierFee, uint96 rewardPerFollow, uint256 sigDeadline, bytes sig, string target_username, uint48 deadline` | A | 创建 campaign。存入 `grossAmount` USDC（= protocolFee + budget）。要求 `sigDeadline >= deadline`（verifier 承诺必须覆盖 campaign） |
...
| 7 | `sigDeadline >= deadline`（verifier 承诺覆盖 campaign） | SignatureExpired |
