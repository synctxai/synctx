...
- **支付代币：** USDC
- **标签：** `["x", "follow"]`
- **身份：** `TwitterRegistry` 绑定为强制要求 — B 的钱包必须绑定 Twitter 用户名
...
struct Deal {
    ...
    uint96  verifierFee;         // 12 字节 — 每次验证的费用（从预算中扣除）
...
| 方法 | 参数 | 调用者 | 说明 |
|------|------|--------|------|
| `createDeal(...)` | `uint96 grossAmount, address verifier, uint96 verifierFee, uint96 rewardPerFollow, uint256 sigDeadline, bytes sig, string target_username, uint48 deadline` | A | 创建 campaign。存入 `grossAmount` USDC（= protocolFee + budget）。预算须 ≥ 1 ×（rewardPerFollow + verifierFee） |
| `claim(dealIndex)` | `uint256 dealIndex` | 任何已认证的 B | 领取关注奖励。从 TwitterRegistry 读取 B 的用户名。从预算锁定（rewardPerFollow + verifierFee）。自动触发验证 |
| `onVerificationResult(...)` | `uint256 dealIndex, uint256 claimIndex, int8 result, string reason` | Verifier | 回调：result>0 → 付款给 B；result<0 → 奖励退回预算；result==0 → 全部退回预算 |
| `withdrawRemaining(dealIndex)` | `uint256 dealIndex` | A | deadline 到期 + 无待处理 claim 后，A 提取剩余预算。Deal → CLOSED |
...
| `canClaim(dealIndex, addr)` | `bool` | addr 是否可 claim（已认证、未领取、预算充足、未过期） |
...
Verifier 每个 campaign 签名一次（不是每个 claim）。此签名承诺：
- 为 `target_username` 验证关注关系
- 每次验证收费 `fee`
- 有效期至 `deadline`
...
specParams = abi.encode(
    string follower_username,  // B 的用户名（从 TwitterRegistry 读取）
...
sequenceDiagram
    ...
    Note over B,D: 领取阶段（可重复，无需协商）

    B->>D: 🟡 canClaim(dealIndex, B.address) → true/false
    Note over B: 在 X 上关注 target_username

    B->>D: 🟢 claim(dealIndex)
    Note over D: 读取 TwitterRegistry.usernameOf[B] → follower_username<br/>从预算锁定（rewardPerFollow + verifierFee）<br/>🔵 VerificationRequested(dealIndex, claimIndex, verifier)
    ...
    alt result > 0（已关注）
        ...
        Note over D: reward → 预算，verifierFee → verifier<br/>B 无损失（A 承担所有费用）
    else result == 0（不确定）
        Note over D: reward + verifierFee → 预算<br/>无人承担成本
    end

    Note over A,D: 提取阶段（deadline 到期后）

    A->>D: 🟢 withdrawRemaining(dealIndex)
    Note over D: 剩余预算 → A<br/>🔵 DealStateChanged(dealIndex, CLOSED)
```
...
> A 仅在 `pendingClaims == 0` 且 `block.timestamp > deadline` 时可提取。
...
| B 无惩罚 | 失败的 claim 奖励退回预算；B 无需支付任何费用 |
| 身份强制要求 | TwitterRegistry 绑定为必须 — 防止女巫攻击/冒领 |
...
| 结果 | 奖励（rewardPerFollow） | Verifier 费用 | 预算变化 |
|------|------------------------|---------------|---------|
| 通过 (result > 0) | → B | → Verifier | — |
| 失败 (result < 0) | → 预算 | → Verifier | +rewardPerFollow |
...
| 3 | 链上费用 > 0 | 不是则跳过 |
| 4 | TwitterRegistry 已配置（签名阶段） | 未配置则拒绝 |
