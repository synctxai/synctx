...
| `claim(dealIndex)` | `uint256 dealIndex` | 任何 B | B 只传 `dealIndex`。合约读取 `TwitterRegistry.usernameOf[msg.sender]` — 未绑定则 revert `NotVerified`。从预算锁定 `claimCost()`（= rewardPerFollow + verifierFee + protocolFee）。发出 VerificationRequested |
| `onVerificationResult(...)` | `uint256 dealIndex, uint256 claimIndex, int8 result, string reason` | Verifier | result>0 → 付款给 B + verifier + FeeCollector，completedClaims++；result<0 → 奖励退回预算，verifier + FeeCollector 照付，failCount[B]++；result==0 → 全部退回预算 |
...
| `canClaim(dealIndex, addr)` | `bool` | addr 是否可 claim（有 TwitterRegistry 绑定、未领取、预算 ≥ claimCost、未过期） |
